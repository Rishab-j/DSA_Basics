...
public class Heap {
    ...
    public static void createHeap(int[] dataSet, boolean isMax) {
        ...
        int ei = arr.size() - 1;
        // ek ek krke hrr element ki property check krenge
        for (int i = ei; i >= 0; i--) {
            downHeapify(i, ei);
        }
    }
    ...
    public static void downHeapify(int parIdx, int ei) { // logn
        ...
        if (lci <= ei && arr.get(lci) > arr.get(maxidx))
            maxidx = lci;
        if (rci <= ei && arr.get(rci) > arr.get(maxidx))
            maxidx = rci;

        if (parIdx != maxidx) {
            swap(parIdx, maxidx);    // max ko parent bna do
            downHeapify(maxidx, ei); // agr left child bda tha to maxidx left child idx ko point krr rha hoga and it
    ...
    public static void upheapify(int ci) { // logn
        int pi = (ci - 1) / 2;
